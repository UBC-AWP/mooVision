...
bash scripts_sockeye/01_setup.sh

# 2. Submit the read and split data job and capture its SLURM Job ID number
READ_DATA_MSG=$(sbatch scripts_sockeye/02_read_and_split_data.sh)
...
PREPROCESS_MSG=$(sbatch --dependency=afterok:$READ_DATA_JOB_ID scripts_sockeye/03_preprocessing.sh)
...
TRAIN_MSG=$(sbatch --dependency=afterok:$PREPROCESS_JOB_ID scripts_sockeye/04_train_yolo.sh)
...
BASELINE_INFER_MSG=$(sbatch --dependency=afterok:$TRAIN_JOB_ID scripts_sockeye/05_baseline_testing.sh)
...
YOLO_INFER_MSG=$(sbatch --dependency=afterok:$TRAIN_JOB_ID scripts_sockeye/06_yolo_testing.sh)
...
BASELINE_EVAL_MSG=$(sbatch --dependency=afterok:$BASELINE_INFER_JOB_ID scripts_sockeye/07_baseline_evaluation.sh)
...
YOLO_EVAL_MSG=$(sbatch --dependency=afterok:$YOLO_INFER_JOB_ID scripts_sockeye/08_yolo_evaluation.sh)
