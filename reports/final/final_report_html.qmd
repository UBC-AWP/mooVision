...
: Experimental train/test split design {#tbl-splits tbl-colwidths="[20,10,70]"}
...
![Illustration of Intersection over Union (IoU) between two detected calf bounding boxes. The red highlighted region indicates the intersection area, while the union area encompasses both boxes combined.](img/baseline_iou.png){#fig-baseline-iou width=80%}
...
Current detection performance is not yet suitable for deployment. Neither model achieved useful recall, with detected events falling well short of ground-truth counts (~1,100 events per split), indicating that missed detections remain the primary challenge.
For the AWP's goal of reducing annotation burden, the next priority should be improving model performance through expanded training data, particularly hard negative examples (e.g. non-cross-sucking footage of calves in close proximity), and hyperparameter tuning (e.g. confidence threshold). 
